...
# Рассылка
@dp.message_handler(IsAdmin(), text="📢 Уведомление", state="*")
async def functions_ad(message: Message, state: FSMContext):
    await state.finish()

    await state.set_state("here_ad_text")
    await message.answer("<b>📢 Введите текст для уведомления пользователям.</b>\n"
...
async def functions_ad_send(call: CallbackQuery, state: FSMContext):
    send_message = (await state.get_data())['here_ad_text']
    get_action = call.data.split(":")[1]
    await state.finish()

    if get_action == "yes":
        await call.message.edit_text(f"<b>📢 Уведомление отправляется...</b>")
        asyncio.create_task(functions_ad_make(send_message, call.from_user.id))
    else:
        await call.message.edit_text("<b>📢 Вы отменили отправку уведомлений</b>")


# Отправка рассылки
async def functions_ad_make(message, user_id):
    ...
    await bot.send_message(
        user_id,
        f"<b>📢 Уведомление было отправлено</b>\n"
...
@dp.callback_query_handler(IsAdmin(), text_startswith="user_purchases", state="*")
...
@dp.callback_query_handler(IsAdmin(), text_startswith="user_balance_add", state="*")
...
@dp.callback_query_handler(IsAdmin(), text_startswith="user_balance_set", state="*")
...
@dp.callback_query_handler(IsAdmin(), text_startswith="user_refresh", state="*")
...
async def functions_ad_get(message: Message, state: FSMContext):
    await state.update_data(here_ad_text="🔔 RIM - Магазин Discord Nitro:\n" + str(message.text))
    ...
    try:
        ...
        await message.answer(
            f"<b>📢 Отправить <code>{len(users)}</code> пользователям сообщение?</b>\n"
            f"{message.text}",
        ...
        await message.answer("<b>❌ Ошибка синтаксиса HTML.</b>\n"
                             "📢 Введите текст для уведомлений пользователям.\n"
                             "❕ Вы можете использовать HTML разметку.")


# Принятие айди или логина для поиска профиля
@dp.message_handler(IsAdmin(), state="here_profile")
async def functions_profile_search(message: Message, state: FSMContext):
...
async def functions_profile_balance_add_get(message: Message, state: FSMContext):
    if message.text.isdigit():
        if 0 <= int(message.text) <= 1000000000:
            ...
            await message.answer(f"<b>✅ Пользователю "
                                 f"<a href='tg://user?id={get_user['user_id']}'>{get_user['user_name']}</a> "
                                 f"выдано <code>{message.text}₽</code></b>")

            await message.bot.send_message(user_id, f"<b>💰 Администратор выдал вам <code>{message.text}₽</code></b>")
...
@dp.callback_query_handler(IsAdmin(), text_startswith="user_message", state="*")
